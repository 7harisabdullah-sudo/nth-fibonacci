...
size_t fibonacci(size_t n) {
    ...
    for (size_t i = 0; i < n / 2; i++) { //we already have one batch cooked initially, so we go i < n / 2 (instead of i <= n / 2).
        ...
        array[1] += array[0]; //array[1] = f[k + 1] and array[1] turns to f[k + 1] + f[k + 2] i.e. f[k + 3] //note that array[0] turned tp f[k + 2] just a while ago.
    ...
    //if n is odd, we perform (n - 1) / 2 iterations. //array = {f[n - 1], f[n]}
